...
1. Fluid 会根据 Operation Pod 所在的节点，注入`kubernetes.io/hostname`、`topology.kubernetes.io/zone`、`topology.kubernetes.io/region`信息；

2. 如果用户在 Operation Spec 中定义了 NodeAffinity，Fluid 也会根据 key 去 Pod 所在的 Node的labels 去匹配并记录，如果不存在则不记录；
   - 如果 NodeSelectorOperator 是`In`/`NotIn`，则记录 key 和 value；
   - 如果 NodeSelectorOperator 是`Exists`/`DoesNotExist`，则记录 key，value 是""（空字符串）；
   - 如果 NodeSelectorOperator 是`LT`/`GT`，则记录 key 和 value（node上该标签的值）；

- <font color='red'>如果 Operation 是个分布式作业，具备多个Pod，则不记录运行时的节点信息；</font>
...
- $PodAffinity(Op) = Op.Spec.Affinity + MatchedAffinity(Op, Referent\_Op.Status.NodeAffinityLabels))$


```go
func MatchedAffinity(current DataOperation, prevOpNodeLabels map[string]string) *corev1.Affinity {
    ...
    if policy == "require" {
        ...
        for _, label := range(current.Spec.RunAfter.AffinityStratgey.Require) {
            ...
            affinityTerms := getMatchedAffinity(label, prevOpNodeLabels)
    ...
    if policy == "prefer" {
        ...
        for _, prefer := range(current.Spec.RunAfter.AffinityStratgey.Prefer) {
            // 根据 label name 匹配相关的 affinity
            affinityTerms := getMatchedAffinity(prefer.Name, prevOpNodeLabels)
